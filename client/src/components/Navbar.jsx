...
const Navbar = () => {
    ...
    return (
        <div className='flex items-center justify-between py-4'>
            <Link to='/'>
                <img src={assets.logo} alt='' className='w-28 sm:w-34 lg:w-50'></img>
            </Link>
            <div>
                {
                user ? 
                ...
                : 
                <div className='flex items-center gap-2 sm:gap-5 px-5'>
                    <p onClick = {()=>navigate('/buy')} className='cursor-pointer'>Pricing</p>
                    <button className='bg-zinc-800 text-white px-7
                    py-2 sm:px-7 text-sm rounded-full' onClick={()=>setShowLogin(true)}>Login</button>
                </div>
                }
